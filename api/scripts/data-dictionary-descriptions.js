...
const COLUMNS = {
  ...
  'rest_log.duration_ms':
    'เวลาที่ใช้ทั้งคำขอ หน่วยมิลลิวินาที · วัดด้วยนาฬิกาแบบ monotonic จึงไม่เพี้ยนหากเวลาระบบถูกปรับกลางคัน',
  'rest_log.request_date': 'เวลาที่คำขอเข้ามา (UTC) ความละเอียดมิลลิวินาที · ใช้ตอบว่า “ตอนนั้นเกิดอะไรขึ้น”',
  'rest_log.response_date':
    'เวลาที่ตอบกลับเสร็จ (UTC) · คำนวณจาก `request_date` บวก `duration_ms` จึงไม่มีทางขัดแย้งกันเอง และมี CHECK บังคับว่าต้องไม่มาก่อน `request_date`',
  ...
  'rest_log.created_at':
    'เวลาที่แถวถูกเขียนลงฐานข้อมูล (UTC) · **ไม่ใช่ค่าเดียวกับ `response_date`** เพราะเขียนหลังส่ง response แล้ว ส่วนต่างของสองค่านี้คือความหน่วงของตัวระบบบันทึกเอง ซึ่งเป็นสิ่งแรกที่ควรดูเมื่อ log เริ่มหาย · ไม่มี `updated_at` เพราะเขียนอย่างเดียว',
