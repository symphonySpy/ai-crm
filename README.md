...
> **ระบบที่ติดตั้งไว้รัน AI copilot ในโหมด `mock`** — ไม่มีการเรียกโมเดลจริง
> คำแนะนำที่เห็นบนหน้าจอเป็นข้อความตัวอย่างที่ผ่าน schema และการตรวจความสอดคล้องชุดเดียวกับของจริง
> โค้ดที่เรียก Claude จริงเขียนไว้ครบแล้ว เปิดได้ด้วยตัวแปรสองตัว — ดูหัวข้อ 8 ข้อ 1

### คลิป walkthrough

ทุกฉากทำบนระบบ production จริง ข้อความ LINE ส่งจากมือถือจริง

| เวลา | ฉาก |
|---|---|
| 0:07 | คิวรอคัดกรอง |
| 0:19 | ลูกค้าทัก LINE OA → lead ใหม่เข้าคิวเองโดยไม่ต้อง refresh |
| 0:30 | พนักงานขายรับ lead |
| 0:33 | ขอคำแนะนำจากผู้ช่วย AI (โหมด `mock` — ไม่ได้เรียกโมเดลจริง) |
| 0:36 | ใช้ร่างจาก AI แล้วแก้เอง → อนุมัติและส่งกลับทาง LINE (0:47) |
| 0:57 | ลูกค้าตอบกลับ ข้อความขึ้นบนหน้าจอเอง |
| 1:06 | เอกสารบน GitHub และ AI skill (`SKILL.md`) พร้อม evaluation cases |
...
- **AI copilot บน prod เป็นโหมด `mock`** ตั้งใจไม่ผูก API key ที่มีค่าใช้จ่ายกับงานประเมิน
  การทำงานของโหมดจริงตรวจด้วย eval (`npm run test:skill`) และเทสกรณี AI ล่ม แทนการเรียกโมเดลบน prod
...
1. **เปิด AI copilot โหมดจริง**
   - ตั้งค่าที่ service ของ API: `AI_ADAPTER=live` · `AI_API_KEY=<key จาก console.anthropic.com>` · `AI_MODEL=claude-sonnet-5`
     (API คิดเงินแยกจาก Claude Pro/Max ต้องเติมเครดิตในบัญชี API ต่างหาก)
   - ก่อนใส่ key: ตั้ง **spend limit** และ**การแจ้งเตือนการใช้งาน**ใน Console · ใช้ key แยกเฉพาะระบบนี้ · ปิดการเติมเงินอัตโนมัติ
   - ยืนยันด้วย Deploy Logs ว่าขึ้น `AI adapter: live`
   - **ต้องทำคู่กัน — รับมือเครดิตหมดหรือ key ใช้ไม่ได้** ตอนนี้ระบบตกไปใช้กติกาสำรองได้ถูกต้อง
     แต่ log เป็นแค่ `AI provider error 400` ระดับ warn และยังยิงไปหา Anthropic ทุกครั้งที่มีคนกด
     - แยกเหตุ "เครดิตหมด / key ถูกปฏิเสธ" ออกจาก error ทั่วไป และ log ระดับ error พร้อมเหตุผลที่อ่านแล้วรู้ว่าต้องทำอะไร
     - circuit breaker: ล้มด้วยเหตุที่ลองใหม่ก็ไม่หาย ให้หยุดเรียก 5 นาทีแล้วใช้กติกาสำรองทันที ครบเวลาลองใหม่หนึ่งครั้ง
       เติมเงินแล้วระบบกลับมาใช้ได้เองโดยไม่ต้อง deploy
     - บอกพนักงานขายตรง ๆ ว่า "ผู้ช่วย AI ใช้ไม่ได้ชั่วคราว คะแนนนี้คำนวณจากกฎ"
     - `GET /health/ai` ให้ตัวตรวจภายนอกเช็คแล้วแจ้งเตือนเมื่อ AI ใช้ไม่ได้
   - ประเมินด้วยบทสนทนาจริงที่ปิดข้อมูลส่วนบุคคลแล้ว ก่อนให้พนักงานเชื่อคะแนน
     และแจ้งในนโยบาย PDPA ว่าบทสนทนาถูกส่งไปประมวลผลที่ผู้ให้บริการภายนอก
2. ย้าย session ไปเก็บฝั่ง server เพื่อให้สั่ง logout ทุกอุปกรณ์ได้
3. ใส่ job queue ให้การส่ง LINE และการเรียก AI แยกจาก request ของผู้ใช้
4. เพิ่มสิทธิ์ระดับทีม/ภูมิภาค แทนที่จะเห็น pipeline ทั้งบริษัททุกคน
5. ต่อ metric และ alert กับ `rest_log` และสถานะ `degraded` ของ AI
6. ทำ data retention ให้ `rest_log` และ `tbl_audit_history` อัตโนมัติ (ตอนนี้เป็นสคริปต์)
7. **ให้ polling ดึงเฉพาะของใหม่** — ตอนนี้ทุก 10 วินาทีหน้าบทสนทนาดึง 50 ข้อความล่าสุด
   และ timeline 100 รายการกลับมาทั้งชุด แม้ส่วนใหญ่จะไม่มีอะไรเปลี่ยน
   ควรเปลี่ยนเป็น `?after=<ข้อความล่าสุดที่หน้าจอมีอยู่>` ซึ่งปกติจะได้กลับมา 0 แถว
   ข้อควรระวังตอนทำคือข้อความขาออกที่ **สถานะเปลี่ยน** (`pending` → `sent` / `failed`)
   ไม่ได้เป็นแถวใหม่ จึงต้องถามจาก `updated_at` ด้วย ไม่ใช่ `created_at` อย่างเดียว
   ไม่งั้นหน้าจอจะค้างที่ `pending` ทั้งที่ส่งถึงลูกค้าแล้ว
8. **ย้ายจาก polling ไป SSE** เมื่อวัดได้ว่า polling เป็นภาระของฐานข้อมูลจริง (A46)
   โดยข้อ 7 ควรทำก่อน เพราะถูกกว่ามากและอาจทำให้ไม่ต้องไปถึงข้อนี้เลย
